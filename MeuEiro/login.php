...
if (isset($_POST['login'])) {
    if ($_POST['email'] == '') {
        ...
        if ($_POST['password'] == '') {
            ...
            if ($stmt = $mysqli->prepare("SELECT UserId, FirstName, LastName, Email, Password from user WHERE Email = ? AND Password = ? ")) {
                $stmt->bind_param("ss", $Email, $Password);
                $stmt->execute();
                $stmt->bind_result($UserId_, $FirstName_, $LastName_, $Email_, $Password_);
                $stmt->store_result();
                $stmt->fetch();
...
<head>
    ...
    <title>MeuEiro - Gerenciador financeiro | Login</title>
            ...
            <div class="col-md-4 col-md-offset-4">
                <div class="login-panel panel panel-primary">
                    <div class="panel-heading">
                        <h3 class="panel-title text-center"><span class="glyphicon glyphicon-lock"></span>  <?php echo $UserSign; ?></h3>
                    </div>
                    <div class="panel-body">
                        ...
                        <form method="post" action="" role="form">
                            <fieldset>
                               ...
                               <hr>
                                <button type="submit" name="login" class="btn btn-success btn-block"><span class="glyphicon glyphicon-log-in"></span>  <?php echo $SignIn; ?></button>                                 <hr>
                               <a href="signUp.php" class="btn btn-info btn-block"> <span class="glyphicon glyphicon-pencil"></span> <?php echo $RegisterAnAccount; ?></a>
                            </fieldset>
                        </form>
                    </div>
                </div>
